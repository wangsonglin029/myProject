...
void PCA_CaptureInit(PCA_TypeDef* PCAx, PCA_ICInitTypeDef* PCA_ICInitStruct)
{
    ...
    else if(PCA_ICInitStruct->PCA_Channel == PCA_Channel4)
    {
...
void PCA_SetPwm_output(PCA_TypeDef* PCAx, \
                       PCA_Channel channel, \
                       FunctionalState OutputPolarityReversal, \
                       uint8_t valuel, \
                       uint8_t valueh, \
                       FunctionalState NewStatus)
{
    ...
    if(channel == PCA_Channel0)
    {
        ...
        if(NewStatus == ENABLE)
        {
            ...
            PCAx->POCR &= ~(PCA_OUTPUT_ENABLE << PCA_Channel0);
            PCAx->POCR |= (PCA_OUTPUT_ENABLE << PCA_Channel0);
            if(OutputPolarityReversal == ENABLE) {
...
void PCA_SetPwm_Puls(PCA_TypeDef* PCAx, PCA_Channel channel, uint8_t puls)
{
    if(channel == PCA_Channel0)
    {
        PCAx->CCAP0H = (uint8_t)puls;
    }
    else if(channel == PCA_Channel1)
    {
        PCAx->CCAP1H = (uint8_t)puls;
    }
    else if(channel == PCA_Channel2)
    {
        PCAx->CCAP2H = (uint8_t)puls;
    }
    else if(channel == PCA_Channel3)
    {
        PCAx->CCAP3H = (uint8_t)puls;
    }
    else if(channel == PCA_Channel4)
    {
        PCAx->CCAP4H = (uint8_t)puls;
    }
}

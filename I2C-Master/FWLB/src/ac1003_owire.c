...
void OWIRE_Init(OWIRE_TypeDef* WIREx, OWIRE_InitTypeDef* WIRE_InitStruct)
{
    ...
    temp = (1.0 * (ApbClock / 1000000 / clkdiv * (WIRE_InitStruct->WIRE_RESCNT)) + 0.5);
    WIREx->RSTCNT = temp;
    /*Set the vlaue to the PRESCNT register*/
    temp = (1.0 * (ApbClock / 1000000 / clkdiv * (WIRE_InitStruct->WIRE_PRESCNT)) + 0.5);
    WIREx-> PRESCNT  = temp;

    /*Set width of bit_pulse*/
    temp = (1.0 * (ApbClock / 1000000 / clkdiv * (WIRE_InitStruct->WIRE_BITRATECNT)) + 0.5);

    WIREx->BITRATECNT = temp;

    /*Set count of driver*/
    temp = (1.0 * (ApbClock / 1000000 / clkdiv * (WIRE_InitStruct->WIRE_DRVCNT)) + 0.5);
    WIREx->DRVCNT = temp;

    /*Set time of read for master*/
    temp = (1.0 * (ApbClock / 1000000 / clkdiv * (WIRE_InitStruct->WIRE_RDSMPCNT)) + 0.5);
    WIREx->RDSMPCNT = temp ;

    /*Set the vlaue to  RECCNT register*/
    temp = (1.0 * (ApbClock / 1000000 / clkdiv * (WIRE_InitStruct->WIRE_RECCNT)) + 0.5);
    WIREx->RECCNT = temp;
}
